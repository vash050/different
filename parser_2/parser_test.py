"""парсер задание с фриланса, тренировка"""
import time
import dataset
import requests
...
db = dataset.connect('sqlite:///parser_goods.sqlite3')
table = db['goods']
...
def get_url_page_catalog_list(list_url_pages):
    ...
    try:
        ...
        for url_page in list_url_pages:
            url_all_dist.setdefault(url_page, [])
            url_page_tmp = f'https://www.oboilux.ru/wallpapers{url_page}'
            a = get_url_page_elem_catalog(get_html(url_page_tmp))
            url_all_dist[url_page].extend(a)
        return url_all_dist
    except Exception as e:
        print(e)
...
def get_data_good_from_page(html_page_good):
    ...
    try:
        ...
        price = soup.find("div", class_="col-right-right ci-description").find("div", class_="ci-price").find(
            "span").text
        view = soup.find("div", class_="ci-type-wallpaper").find("span", class_="pic-inroll pic-img-type").text
        fastness_to_light = soup.find("div", class_="ci-type-wallpaper").find("span",
                                                                              class_="pic-inroll pic-img-light").text
        resistance_to_friction = soup.find("div", class_="ci-type-wallpaper").find("span",
                                                                                   class_="pic-inroll pic-img-wash").text
        width = soup.find("div", class_="ci-type-wallpaper").find("span", class_="pic-inroll pic-img-width").text
        repeat_drawing = soup.find("div", class_="ci-type-wallpaper").find("span",
                                                                           class_="pic-inroll pic-img-repeat").text
        ...
        results_data_dict = {
            'country': country, 'brand': brand, 'collection': collection,
            'article': article, 'price': price, 'view': view,
            'fastness_to_light': fastness_to_light, 'resistance_to_friction': resistance_to_friction, 'width': width,
            'repeat_drawing': repeat_drawing, 'length': length, 'description': description,
...
def write_in_bd(data_good):
    """функция принимает данные товара и записывает их в бд"""
    try:
        table.insert(data_good)
    except Exception as ex:
        print(ex)
...
def get_data_goods(dist_url_page_all):
    ...
    """
    for key, val in dist_url_page_all.items():
        for el in val:
            try:
                print(el)
                link = f'https://www.oboilux.ru/wallpapers{el}'
                data_good_from_page = get_data_good_from_page(get_html(link))
                write_in_bd(data_good_from_page)
            except Exception as e:
                print(e)
                continue
...
def main():
    url_site_page_start = 'https://www.oboilux.ru/wallpapers'
    url_next_page_lists = get_data(get_html(url_site_page_start))
